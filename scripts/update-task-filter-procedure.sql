-- Drop and recreate the stored procedure with better NULL handling
...
BEGIN
    ...
    ELSIF cutoff_time IS NOT NULL THEN
        ...
        AND (
            ...
            (
                t.status = 'completed' 
                AND (
                    (t.completed_at IS NOT NULL AND t.completed_at >= cutoff_time)
                    OR 
                    (t.completed_at IS NULL AND t.updated_at >= cutoff_time)
                )
            )
        )
        ORDER BY t.created_at DESC;
    ...
    RAISE NOTICE 'Returning % tasks', task_count;
