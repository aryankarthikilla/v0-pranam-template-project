  ...
  try {
    ...
    const { data: task, error } = await supabase
      ...
      .insert({
        ...
        description: `${suggestion.description}\n\nAI Reasoning: ${suggestion.reasoning || "Smart suggestion"}`,
        ...
        is_deleted: false,
        level: 1,
        ai_priority_value: suggestion.priority === "high" ? 80 : suggestion.priority === "medium" ? 60 : 40,
    ...
    await supabase.from("task_notes").insert({
      task_id: task.id,
      user_id: user.id,
      note_text: `Smart suggestion created: ${suggestion.reasoning || "AI-generated opportunistic task"}`,
...
export async function createAndStartTaskFromSuggestion(
  ...
  try {
    ...
    const startResult = await startTaskSession(createResult.task.id, locationContext, "web")
    ...
    return { success: false, error: error instanceof Error ? error.message : "Failed to create and start task" }
  }
}
