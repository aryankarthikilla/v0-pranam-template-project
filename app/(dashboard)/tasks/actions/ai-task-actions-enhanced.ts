...
import {
  ...
  prioritizeExistingTasks,
  suggestPriority,
} from "@/lib/ai/gemini-enhanced";
...
export async function prioritizeMyTasks() {
  console.log(
    "🚀 Server Action: Prioritizing existing tasks (excluding scheduled)"
  );
  ...
  try {
    ...
    console.log(
      `📊 Filtered ${tasks.length} total tasks to ${availableTasks.length} available tasks`
    );
    ...
    const prioritization = await prioritizeExistingTasks(
      availableTasks,
      user.id
    );
...
export async function deleteOldAILogs() {
  try {
    ...
    return {
      success: false,
      error:
        error instanceof Error ? error.message : "Failed to delete old logs",
    };
  }
}
...
export async function suggestTaskPriority(
  ...
  try {
    ...
    const priorityData = await suggestPriority(
      taskTitle,
      taskDescription,
      dueDate,
      user.id
...
`;
    ...
    console.error(
      "❌ Server Action: Generate opportunistic tasks error:",
      error
    );
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : "Failed to generate opportunistic tasks",
      suggestions: [],
    };
  }
}
