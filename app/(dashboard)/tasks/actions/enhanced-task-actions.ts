...
export async function fixOrphanedTasks() {
  ...
  try {
    ...
    if (tasksError) {
      console.error("❌ Failed to get active tasks:", tasksError)
      return { success: false, error: "Failed to get active tasks" }
    }

    console.log("📋 Found active tasks:", activeTasks?.length || 0)

    if (!activeTasks || activeTasks.length === 0) {
      return { success: true, message: "No active tasks found" }
    }

    // Check each active task for corresponding sessions
    const orphanedTasks = []
    ...
    for (const task of activeTasks) {
      ...
      if (sessionError) {
        console.error("❌ Failed to check sessions for task:", task.id, sessionError)
        continue
      }

      if (!sessions || sessions.length === 0) {
        console.log("🚨 Found orphaned task:", task.title)
    ...
    if (orphanedTasks.length === 0) {
      return { success: true, message: "No orphaned tasks found" }
    }
    ...
    if (resetError) {
      ...
      return { success: false, error: "Failed to reset orphaned tasks" }
    }

    console.log("✅ Successfully reset orphaned tasks:", orphanedTasks.length)

    revalidatePath("/tasks")
    return {
      success: true,
      message: `Fixed ${orphanedTasks.length} orphaned task${orphanedTasks.length > 1 ? "s" : ""}`,
      fixedTasks: orphanedTasks.map((t) => ({ id: t.id, title: t.title })),
    }
  } catch (error) {
    console.error("💥 Exception in fixOrphanedTasks:", error)
